...
key_path="${private_dir_path}/root-ca.key" # 秘密鍵ファイル
csr_path="${wk_dir_path}/root-ca.csr"      # 証明書署名要求ファイル
crt_path="${wk_dir_path}/root-ca.crt"      # 証明書ファイル
...
#   -extensions: 証明書に追加する拡張設定（root-ca.conf内で定義）
openssl ca -selfsign -batch -config "./root-ca.conf" \
    -in "${csr_path}" \
    -out "${crt_path}" \
    -extensions req_ext
